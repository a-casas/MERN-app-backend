  ...
  User.findById(userID)
    .then((user) => {
      ...
      User.findOneAndUpdate(
        { _id: userID },
        { wantToVisit: wantToVisit, $push: { alreadyVisited: poi } }
      ).then((result) => {
...
router.post("/from-alreadyVisited", (req, res, next) => {
  ...
  User.findById(userID)
    .then((user) => {
      ...
      User.findOneAndUpdate(
        { _id: userID },
        { alreadyVisited, $push: { wantToVisit: poi } }
      ).then((result) => {
        res.status(200).json(result);
      });
    })
...
router.post("/from-want-to-visit/delete/:poi", (req, res, next) => {
  ...
  User.findById(userID)
    .then((user) => {
      ...
      const { wantToVisit } = user;
      wantToVisit.splice(wantToVisit.indexOf(poi), 1);
      User.findOneAndUpdate({ _id: userID }, { wantToVisit }).then((result) => {
        res.status(200).json(result);
      });
    })
    .catch((err) => console.error(err));
...
router.post("/from-visited/delete/:poi", (req, res, next) => {
  ...
  User.findById(userID)
    .then((user) => {
      ...
      const { alreadyVisited } = user;
      alreadyVisited.splice(alreadyVisited.indexOf(poi), 1);
      User.findOneAndUpdate({ _id: userID }, { alreadyVisited }).then(
        (result) => {
          res.status(200).json(result);
        }
      );
...
router.post("/hotel/delete/:hotel", (req, res, next) => {
  ...
  User.findById(userID)
    .then((user) => {
      ...
      User.findOneAndUpdate({ _id: userID }, { hotelsBooking }).then(
        (result) => {
          res.status(200).json(result);
        }
      );
